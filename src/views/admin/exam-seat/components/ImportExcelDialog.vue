  ...
  >
    ...
    <template #footer v-if="preview.length">
      <div class="import-footer">
        <label class="import-footer-check">
          <input type="checkbox" v-model="groupByClass" />
          <span>同班随机排列至同一教室</span>
        </label>
        <span class="import-footer-hint" title="同班学生进度不同、试卷不同，开启后自动将同班学生安排在同一考场">?</span>
...
function doImport() {
  ...
  if (groupByClass.value && result.added.length > 0) {
    store.runAutoArrange({ groupByClass: true })
    ElMessage.success(`已导入 ${result.added.length} 名学生并自动按班级分教室排座（共 ${result.total} 人）`)
  } else {
