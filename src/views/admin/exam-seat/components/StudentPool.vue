...
function initSortable() {
  ...
  sortable = Sortable.create(poolEl.value, {
    ...
    onEnd(evt) {
      ...
      const clientX = evt.originalEvent.clientX ?? evt.originalEvent.changedTouches?.[0]?.clientX
      const clientY = evt.originalEvent.clientY ?? evt.originalEvent.changedTouches?.[0]?.clientY
      if (clientX == null || clientY == null) return

      const targetEl = document.elementFromPoint(clientX, clientY)
      const targetCell = targetEl?.closest('.sg-cell')
      if (!targetCell) return

      const targetGrid = targetCell.closest('.sg-grid')
      const targetRoomId = targetGrid ? parseInt(targetGrid.dataset.roomId) : null
      const seatIndex = parseInt(targetCell.dataset.seatIndex)
      if (!targetRoomId || isNaN(seatIndex)) return

      // 检查目标是否可用
      if (store.isSeatBlocked(targetRoomId, seatIndex)) return
      const existing = store.assignments.find(a => a.roomId === targetRoomId && a.seatIndex === seatIndex)
