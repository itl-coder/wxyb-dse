    ...
    <div class="conf-layout">
      ...
      <div class="conf-left">
        ...
        <div class="admin-card" v-if="studentProfile">
          <div class="admin-card-title collapsible-title" style="font-size:14px;margin-bottom:0" @click="toggleCard('overview')">
            <span class="collapsible-caret">{{ cardCollapse.overview ? '▸' : '▾' }}</span>
            <span>📊 数据总览</span>
          </div>
          <div v-if="!cardCollapse.overview" class="collapsible-body">
          ...
          </div><!-- end .collapsible-body for overview -->
        </div>

        <div class="admin-card" v-if="studentProfile">
          <div class="admin-card-title collapsible-title" style="font-size:14px;margin-bottom:0" @click="toggleCard('filters')">
            <span class="collapsible-caret">{{ cardCollapse.filters ? '▸' : '▾' }}</span>
            <span>📊 数据筛选</span>
          </div>
          <div v-if="!cardCollapse.filters" class="collapsible-body">
          <div class="admin-form-group">
            <label>考试月份</label>
          ...
          </div><!-- end .collapsible-body for filters -->
        </div>

        <div class="admin-card" v-if="studentProfile">
          <div class="admin-card-title collapsible-title" style="font-size:14px;margin-bottom:0" @click="toggleCard('supplements')">
            <span class="collapsible-caret">{{ cardCollapse.supplements ? '▸' : '▾' }}</span>
            <span>📝 补充材料</span>
          </div>
          <div v-if="!cardCollapse.supplements" class="collapsible-body">
          ...
          </div><!-- end .collapsible-body for supplements -->
        </div>

        <div class="admin-card" v-if="studentProfile">
          <div class="admin-card-title collapsible-title" style="font-size:14px;margin-bottom:0" @click="toggleCard('teacherNotes')">
            <span class="collapsible-caret">{{ cardCollapse.teacherNotes ? '▸' : '▾' }}</span>
            <span>✏️ 教师补充意见</span>
          </div>
          <div v-if="!cardCollapse.teacherNotes" class="collapsible-body">
          <div class="admin-form-group">
          ...
          </div><!-- end .collapsible-body for teacherNotes -->
        </div>

        <div class="admin-card" v-if="studentProfile">
          <div class="admin-card-title collapsible-title" style="font-size:14px;margin-bottom:0" @click="toggleCard('diagnosis')">
            <span class="collapsible-caret">{{ cardCollapse.diagnosis ? '▸' : '▾' }}</span>
            <span>🔬 学情诊断</span>
          </div>
          <div v-if="!cardCollapse.diagnosis" class="collapsible-body">
          ...
          </div><!-- end .collapsible-body for diagnosis -->
        </div>

        <!-- Template Selector -->
        <div class="admin-card" v-if="studentProfile">
          <div class="admin-card-title collapsible-title" style="font-size:14px;margin-bottom:0" @click="toggleCard('template')">
            <span class="collapsible-caret">{{ cardCollapse.template ? '▸' : '▾' }}</span>
            <span>📋 文稿模板</span>
          </div>
          <div v-if="!cardCollapse.template" class="collapsible-body">
          ...
          </div><!-- end .collapsible-body for template -->
        </div>

        <!-- Draggable Module List -->
        <div class="admin-card" v-if="studentProfile">
          <div class="admin-card-title collapsible-title" style="font-size:14px;margin-bottom:0" @click="toggleCard('modules')">
            <span class="collapsible-caret">{{ cardCollapse.modules ? '▸' : '▾' }}</span>
            <span>📝 文稿模块</span>
          </div>
          <div v-if="!cardCollapse.modules" class="collapsible-body">
          ...
          </div><!-- end .collapsible-body for modules -->
        </div>
      </div>
...
/**
 * 页面：家长会准备
 * 功能：按选修科目动态生成个性化家长会文稿，整合多维度学生数据，支持导出图片/PDF
 * 路由：/admin/conference
 */
...
import { buildPrintHTML, getPrintWatermarkStyle, getPrintWatermarkHTML, getOverlayWatermarkHTML } from '@/utils/printTemplate'
...
// Left panel card collapse state (persisted)
const CARD_COLLAPSE_KEY = 'dse_conf_card_collapse'
const cardCollapse = ref({
  overview: false, filters: false, supplements: false,
  teacherNotes: false, diagnosis: false, template: false, modules: false
})
function loadCardCollapse() {
  try {
    const saved = localStorage.getItem(CARD_COLLAPSE_KEY)
    if (saved) Object.assign(cardCollapse.value, JSON.parse(saved))
  } catch {}
}
function toggleCard(key) {
  cardCollapse.value[key] = !cardCollapse.value[key]
  localStorage.setItem(CARD_COLLAPSE_KEY, JSON.stringify(cardCollapse.value))
}
loadCardCollapse()
...
<style scoped>
.conference-page { display: flex; flex-direction: column; height: 100%; min-height: 0; }
/* === Page Hero === */
.conf-page-hero {
  display: flex;
  align-items: center;
...
/* === Layout === */
.conf-layout { display: grid; grid-template-columns: 340px 1fr; gap: 16px; flex: 1; min-height: 0; }

/* Left Panel */
.conf-left { display: flex; flex-direction: column; gap: 16px; overflow-y: auto; }

/* Right Panel */
.conf-right { min-height: 0; overflow: hidden; }
...
@media (max-width: 768px) {
  .conf-layout { grid-template-columns: 1fr; height: auto; }
  .conf-left { max-height: 50vh; }
  .conf-right { min-height: 60vh; }
}
...
.collapsible-title:hover { color: var(--admin-accent); }
.collapsible-caret {
  display: inline-block; width: 14px; font-size: 10px;
  color: var(--admin-text-muted); flex-shrink: 0;
}
