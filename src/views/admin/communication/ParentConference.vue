  ...
  <div>
        ...
        <div v-for="(conf, idx) in filteredList" :key="conf.id" class="pc-timeline-item"
          ...
          <div class="pc-booking-card">
            ...
            <div class="pc-card-meta">
              ...
              <span v-if="conf.meetingId" style="font-size:10px;color:var(--admin-text-muted)">腾讯会议: {{ conf.meetingId }}</span>
    ...
    <el-dialog v-model="dialogVisible" :title="editingId ? '编辑预约' : '新建家长会预约'" width="520px">
      ...
      <div class="admin-three-col" style="margin-bottom:0">
        ...
        <div class="admin-form-group">
          <label>教室</label>
          <el-input v-model="form.room" size="small" style="width:100%" :disabled="form.mode === 'online'" placeholder="请输入教室名称，如：教学楼302会议室" />
        </div>
      </div>
...
/**
 * 页面：家长会预约管理
 * 功能：安排家长面谈预约，支持时间线视图和日历视图，追踪沟通记录与导出预约凭证
 * 路由：/admin/parent-conference
 */
