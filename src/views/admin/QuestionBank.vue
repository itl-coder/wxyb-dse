<template>
  <div class="qb-page">
    ...
    <el-dialog v-model="dialogVisible" :title="editingId ? '编辑题目' : '新增题目'" width="720px" top="3vh" :close-on-click-modal="false">
      ...
      <div class="admin-form-group">
        <label>知识点说明</label>
        <el-input v-model="form.knowledgePoint" placeholder="如：二次函数顶点式 f(x)=a(x−h)²+k" />
      </div>
      <div class="admin-form-group">
        <label>常见错误</label>
        <el-input v-model="form.commonMistakes" placeholder="学生容易犯的错误..." />
      </div>
      <div class="admin-form-group">
        <label>图片附件</label>
        <div style="display:flex;align-items:center;gap:10px">
          <div class="qb-image-upload" @click="triggerImageUpload">
            ...
            <img v-if="form.image" :src="form.image" class="qb-image-preview" />
            <div v-else class="qb-image-placeholder">
              <span style="font-size:24px">📷</span>
              <span style="font-size:10px;color:var(--admin-text-muted)">点击上传</span>
            </div>
          </div>
          <el-button v-if="form.image" size="small" type="danger" @click="form.image = null">移除</el-button>
        </div>
      </div>
      <template #footer>
...
const form = ref({
  ...
  knowledgePoint: '', commonMistakes: '', estimatedRate: 70, image: null
})
...
function openAddDialog() {
  ...
  form.value = { subject: '数学', type: 'mc', difficulty: 'medium', topic: '', year: '', score: 5, suggestedTime: 3, text: '', options: '', answer: '', steps: '', knowledgePoint: '', commonMistakes: '', estimatedRate: 70, image: null }
...
function saveQuestion() {
  ...
  const data = { ...form.value, createdAt: new Date().toISOString().split('T')[0] }
  if (editingId.value) {
